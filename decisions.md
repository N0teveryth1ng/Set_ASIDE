...
| D-017 | Scripted end-to-end verification provisions throwaway users straight into `auth.users` via the direct pooler connection and obtains tokens with `grant_type=password`; no signup, no `auth/v1/admin` calls | The headless `service_role` secret was rotated mid-project (admin endpoints now 403 bad_jwt) and signup is rate-limited/reserved for real users; seeding only (never `auth.identities` — GoTrue auto-creates that on first password login) keeps login screens exercised exactly like a real browser |
